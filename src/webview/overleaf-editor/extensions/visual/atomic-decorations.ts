    ...
    tree.iterate({
      enter(nodeRef) {
        ...
        if (nodeRef.type.is('$Environment')) {
          ...
          nodeRef.type.is('GlossaryReference') ||
          nodeRef.type.is('GlossaryDisplay')
        ) {
          if (shouldDecorate(state, nodeRef)) {
            const argumentNode = nodeRef.node
            ...
            if (nodeRef.type.is('GlossaryDisplay')) {
              const displayNode = nodeRef.node.getChild('TextArgument')
              decorations.push(
                ...decorateArgumentBraces(
                  new BraceWidget(' '),
                  displayNode,
                  displayNode?.from ?? nodeRef.from
                )
              )
            }
          }

          // Glossary keys are plain identifiers, but alternate display text
          // can contain regular visual-editor markup.
          return nodeRef.type.is('GlossaryDisplay')
